    ...
    <div className="min-h-screen bg-white pt-24 pb-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12">
          ...
          <p className="text-gray-500 text-lg">Everything you need to get PostOwl.in working.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-[240px_1fr] gap-12">
            ...
            <nav className="sticky top-24 space-y-1">
              {sections.map((s) => (
                <a
                  key={s.slug}
                  href={s.slug}
                  className="block px-3 py-2 rounded-lg text-sm text-gray-600 hover:text-[#1E1B4B] hover:bg-gray-50 transition-colors"
                >
                  {s.title}
                </a>
              ))}
          ...
          <div className="space-y-16 min-w-0">
            <section id="getting-started">
              <h2 className="text-2xl font-bold text-[#1E1B4B] mb-6">Getting Started</h2>
              <ol className="space-y-4">
                {[
                  ...
                  <li key={i} className="flex gap-4">
                    ...
                    <div>
                      <div className="font-semibold text-gray-900">{s.step}</div>
                      <div className="text-gray-500 text-sm mt-0.5">
                        {s.detail}{" "}
                        {s.link && (
                          <Link href={s.link} className="text-[#1E1B4B] underline">
                            Go to download →
                          </Link>
                        )}
                      </div>
            ...
            <section id="email-setup">
              <h2 className="text-2xl font-bold text-[#1E1B4B] mb-2">Email Account Setup</h2>
              <p className="text-gray-500 text-sm mb-8">
                PostOwl.in works with any email provider. Choose yours below.
              </p>
              <div className="space-y-6">
                {emailProviders.map((p) => (
                  <div key={p.name} className="border border-gray-200 rounded-xl overflow-hidden">
                    <div className="bg-gray-50 px-6 py-4 font-semibold text-gray-900 border-b border-gray-200">
                      {p.name}
                    </div>
            ...
            <section id="contacts">
              <h2 className="text-2xl font-bold text-[#1E1B4B] mb-4">Contacts & Groups</h2>
              <div className="prose prose-gray max-w-none text-sm text-gray-600 space-y-4">
                <p><strong>Importing contacts:</strong> Go to Contacts → Import CSV. PostOwl.in detects all columns. The <code>email</code> column is required; all other columns become personalisation fields.</p>
            ...
            <section id="bulk-send">
              <h2 className="text-2xl font-bold text-[#1E1B4B] mb-4">Bulk Send & Personalisation</h2>
              <div className="text-sm text-gray-600 space-y-4">
                ...
                <p>
                  ...
                  <code className="bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded font-mono">{"{{first_name}}"}</code>,{" "}
                  <code className="bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded font-mono">{"{{company}}"}</code>.
            ...
            <section id="templates">
              <h2 className="text-2xl font-bold text-[#1E1B4B] mb-4">Templates</h2>
            ...
            <section id="logs">
              <h2 className="text-2xl font-bold text-[#1E1B4B] mb-4">Campaign Logs</h2>
              <div className="text-sm text-gray-600 space-y-3">
            ...
            <section id="reference">
              <h2 className="text-2xl font-bold text-[#1E1B4B] mb-4">Keyboard Shortcuts</h2>
                ...
                <table className="w-full text-sm border border-gray-200 rounded-xl overflow-hidden">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      ...
                      <th className="text-left px-4 py-3 font-medium text-gray-600">Windows</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={s.action} className="border-b border-gray-100 last:border-0">
                        <td className="px-4 py-3 text-gray-700">{s.action}</td>
                        <td className="px-4 py-3 font-mono text-gray-600">{s.mac}</td>
                        <td className="px-4 py-3 font-mono text-gray-600">{s.win}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
